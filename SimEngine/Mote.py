...
from MoteTimer import MoteTimer
...
class Mote(object):
    ...
    def __init__(self,id):
        ...
        self.filename                  ="../bin/simData/mote_"+str(self.id)+".csv"
        ...
        self.packetStructure      = []
        self.idleTimes                 =0
        # location
        # battery
        self.chargeConsumed            = 0
        self.activeTime                = 0
        self.chargeConsumedIdle        = 0
        ...
        #=========================
        self.energy =10.0 #initial energy of 10.0 milli Amps
        #======================
                #timing
        self.timeslot=0
        self.awake = False
        self.assignedTimeslots =[]
        self.timer = MoteTimer(self.settings.slotDuration/1000,self.updateTimeSlot)
        self.topology =None
        self.topology_tag=None
        self.mote_log=""
        self.combinedCellList=[]
    #======================== stack ===========================================
    ...
    def _app_schedule_sendSinglePacket(self,firstPacket=False):
        ...
        if not firstPacket:
            ...
            delay            = self.settings.slotDuration*self.settings.slotframeLength
        else:
    ...
    def _rpl_housekeeping(self):
        with self.dataLock:
            ...
            if sorted_potentialRanks:
                ...
                #self.parentSet = [n for ()]
                #get the neighbor with the minimum rank

                #print "neighbors list {0}-{1}, mote-rank = {2}".format(self.id, [(n.id, n.getMoteRank()) for n in self._myNeigbors()], self.getMoteRank())
                self.parentSet = [sorted(self._myNeigbors(), key=lambda mote: mote.getMoteRank())[0]]
            ...
            etxs        = dict([(p, 1.0/(p.rank+self._rpl_calcRankIncrease(p))) for p in self.parentSet])
    ...
    def _increasePacketsToSend(self, increment):
        self.packetsTosend=self.packetsTosend+increment
    ...
    def _sixtop_cell_reservation_response(self,neighbor,numCells,dirNeighbor):
        ...
        with self.dataLock:
            ...
            cells={}
            #availableTimeslots=[]
            #self.combinedCellList is set by LinkSchedules
            #each cell has the timeslot, dir, neighborid, channel
    ...
    def _tsch_enqueue(self,packet):
        ...
        if not self.preferredParent:
            ...
            #print "Packete Dropped due to Parent"
            ...
            # my TX queue is full
            # update mote stats
            #print "Please print me before they drop the packet" +str(len(self.txQueue))
            ...
            # all is good
            # enqueue packet
            #print "This is my packet size after putting in"+ str(len(self.engine.motes[1].txQueue)) + "for mote"+ str(self.engine.motes[1].id)

            self.txQueue    += [packet]
            #print "this is the packet size" +str(len(packet))
            #print "This is my packet size after putting in"+ str(len(self.txQueue)) + "for mote"+ str(self.id)
        ...
        with self.dataLock:
            ...
            if ts in self.schedule:
                ...
                if  cell['dir']==self.DIR_RX:
                    ...
                    self.pktToSend = None
                    #print "This is my queue size at checking when"+ str(len(self.txQueue)) + "for mote"+ str(self.id)
        ...
        with self.dataLock:
            ...
            for cell in cellList:
              try:
                    ...
                    e.args += ( cellList,"ts =", cell[0], " id= ",self.id, "schedule=",self.schedule)
                    raise
    ...
    def radio_txDone(self,isACKed,isNACKed):
        ...
        #print "this is my queue size before" + str(len(self.engine.motes[1].txQueue)) + "for mote "+ str(self.engine.motes[1].id)
        ...
        with self.dataLock:
            ...
            if isACKed:
                ...
                #print "This is my packet size isAcked when removing packets"+ str(len(self.txQueue))

                self.txQueue.remove(self.pktToSend)
                #print "This is my queue size after removing the packets"+ str(len(self.engine.motes[1].txQueue) + "for mote"+ str(self.engine.motes[1].id)


            elif isNACKed:
                # NACK received
                print "i am not ackowledged"
                ...
                if self.txQueue[i]['retriesLeft'] == 0:
                    #print "this is my queue size"+ str(len(self.txQueue))
                ...
                if self.txQueue[i]['retriesLeft'] == 0:
                    ...
                    if  len(self.txQueue) == self.TSCH_QUEUE_SIZE:
                        ...
                        # update mote stats
                        #print "i have dropped this guy"
                        self._stats_incrementMoteStats('droppedMacRetries')

                        # remove packet from queue
                        self.txQueue.remove(self.pktToSend)
                        #print "This is my size due to removing packets"+ str(len(self.txQueue)) + "for mote"+ str(self.id)
    ...
    def radio_rxDone(self,type=None,smac=None,dmac=None,payload=None):
        ...
        with self.dataLock:
            self.timeslot = ts
            self.channel  = self.schedule[ts]["ch"]
            ...
            if smac:
                #print "this is smac " + str(smac)
                # I received a packet
                print "i received a packet {0} at timeslot {1} from {2} and packettype {3}".format(self.id,ts,smac.id,type)

                self.logsmacts(smac,type,ts)
                # log charge usage
                self.activeTime +=1
                ...
                if self.dagRoot:
                    ...
                    # update mote stats
                    print "it has reeached the DAG ROOT"
                ...
                #number of times you have been idlelistening
                self.idleTimes+=1
                # this was an idle listen
                print "i was idle listening " + str(self.idleTimes) + "this is smac " + str(smac)
                # log charge usage
                self._logChargeConsumedIdle(self.CHARGE_Idle_uC)
    ...
    def setDistance(self,neighbor,pdr):
        ''' sets the pdr to that neighbor'''
        with self.dataLock:
            self.DISTANCE[neighbor] = pdr
    ...
    def setMoteRank(self,moterank):
        '''returns the rank of the mote'''
        with self.dataLock:
            self.mote_rank = moterank

    def getMoteRank(self):
        with self.dataLock:
            return self.mote_rank
    ...
    def _myNeigbors(self):
        return [n for n in self.DISTANCE.keys() if self.DISTANCE[n]< 100]
    #===== clock
    ...
    def _logChargeConsumedIdle(self,charge):
        with self.dataLock:
            self.chargeConsumedIdle  += charge


    def logsmacts(self,smac,type,ts):
        with self.dataLock:
            self.packetStructure +=[(smac.id,type,ts)]
    ...
    def getTxCells(self):
        with self.dataLock:
            ...
            x=[(ts,c['ch'],c['neighbor']) for (ts,c) in self.schedule.items() if c['dir']==self.DIR_TX]
    ...
    #===== stats
    def updateTag(self, topology, tag):
    ...
    def getMoteStats(self):
        ...
        with self.dataLock:
            ...
            returnVal['chargeConsumedIdle'] = self.chargeConsumedIdle
    ...
    def assignSchedule(self,schedule,slotFrameSize):
        self.assignedTimeslots=[]
        for cell in schedule:
            self.assignedTimeslots.append(cell[0])
    ...
    def updateTimeSlot(self):
        if self.engine.scheduler.slotFrameSize != 0:
            self.timeslot=self.engine.getAsn()%self.settings.slotframeLength

            self.updateMoteState()
    def reduceEnergy(self):
        if self.id ==0:
            return 0
        else:
            if self.awake:
                self.energy=self.energy-0.0036 #reduce the energy by 3.6mA
            else:
                self.energy=self.energy-0.0010 #reduce the energy by 1.0mA
    def updateMoteState(self):
        if self.timeslot in self.assignedTimeslots:
            self.awake=True
            self._stats_incrementMoteStats('awake')
            if not self.topology==None:
                self.topology.itemconfig(self.topology_tag,fill='green')
        elif self.timeslot+1 in self.assignedTimeslots:
            self.awake=True
            self._stats_incrementMoteStats('idle')
            if not self.topology==None:
                self.topology.itemconfig(self.topology_tag,fill='orange')
        else:
            self.awake=False
            self._stats_incrementMoteStats('sleep')
            if not self.topology==None:
                if self.id ==0:
                    self.topology.itemconfig(self.topology_tag,fill='blue')
                else:
                    self.topology.itemconfig(self.topology_tag,fill='red')
        self.mote_log=self.mote_log+str(self.id)+","+str(self.timeslot)+","+str(self.awake)+","+str(self.energy)+"\n"
        self.reduceEnergy()
