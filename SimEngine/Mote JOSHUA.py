...
class Mote(object):
    ...
    def _rpl_housekeeping(self):
        with self.dataLock:
            ...
            if sorted_potentialRanks:
                ...
                self.parentSet = [sorted(neighbors, key=lambda mote: mote.rank)[0]]

                print self.id,
                print self.parentSet[0].id,
                print min(neighbors, key=lambda mote: mote.rank).id
    ...
    def _myNeigbors(self):
        ...
        return [n for n in self.DISTANCE.keys() if self.DISTANCE[n]< 150]
    ...
    def getTMotes(self):
        with self.dataLock:
            return [c['neighbor'] for (ts,c) in self.schedule.items() if c['dir']==self.DIR_TX]

    def getRMotes(self):
        with self.dataLock:
            return [c['neighbor'] for (ts,c) in self.schedule.items() if c['dir']==self.DIR_RX]
