...
class Topology(object):
    ...
    def createTopology(self):
        ...
        for mote in self.motes:
            ...
            while not connected:
                ...
                for cm in connectedMotes:
                    #distance = self._computeDistance(mote, cm)
                ...
                if numStableNeighbors >= 0 or numStableNeighbors == len(connectedMotes):
                    connected = True
        ...
        # computing the distance from each neighbors
        mote_distances = {}
        for mote in self.motes:
            for m in self.motes:
                if mote==m:
                    ...
                    distance = self._computeDistance(mote,m)
                    mote_rank =self._computeMoteRank(distance)
                    mote.setMoteRank(mote_rank)
                    mote.setDistance(m,distance)
                    m.setDistance(mote,distance)
        ...
        for mote in self.motes:
            ...
            #printing the neighbours of a mote
            for mote in self.motes:
                print "neighbours list {0} = {1}, mote-rank = {2}".format(mote.id, [(n.id,n.rank) for n in mote._myNeigbors()], mote.rank)
                print "parent set {0} = {1} and length {2}".format(mote.id, [n.id for n in mote.parentSet], len(mote.parentSet))
    ...
    def _computeDistance(self,mote,neighbor):
        ...
        i have modified it to increase the spacing in between so it doesnt return in m but in
        '''

        return 100*math.sqrt(
            math.pow((mote.x - neighbor.x),2) +
            math.pow((mote.y - neighbor.y),2)
        )

    def _computeMoteRank(self, motedistance):

        return int(motedistance*256)
